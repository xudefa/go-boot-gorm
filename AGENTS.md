# go-boot-gorm 项目开发规范文档

go-boot-gorm 是一个基于 [github.com/xudefa/go-boot](https://github.com/xudefa/go-boot) 的 GORM ORM 集成模块。本模块将 GORM 无缝集成到 go-boot 的 IoC 容器和自动配置体系中，遵循 go-boot 项目的开发规范。
...
- **集成模块**：go-boot-gorm 是 go-boot 的数据访问层集成，将 GORM 作为 ORM 实现
- **规范继承**：完全遵循 go-boot 的开发规范、命名约定、代码风格

### 1.2 核心职责

- 将 GORM DB 注册为 go-boot 容器中的 Bean（Bean ID: `gormDB`）
- 实现 `data.Transactor` 接口的 GORM 适配器
- 提供泛型 `Repository[T]` 实现 `data.Repository[T]` 接口
- 提供自动配置，通过 `gorm.enabled=true` 条件控制
- 自动注册数据库健康指标（Bean ID: `databaseHealthIndicator`）
...
- **ORM 框架**：集成 GORM（`gorm.io/gorm`）
- **职责边界**：仅负责 GORM 集成，不包含其他业务逻辑
- **示例代码**：统一放在 `examples/` 目录，演示 GORM 集成用法

### 2.2 go-boot-gorm 核心包结构

| 文件 | 说明 | 核心类型 |
|------|------|----------|
| `gorm.go` | GORM 核心实现（DB、Repository、Transaction） | `DB`, `Repository[T]`, `Transaction`, `Client` |
| `gorm_starter.go` | GORM 启动器 | `GormStarter` |
| `autoconfig.go` | GORM 自动配置 | `GormAutoConfiguration` |
| `model.go` | 数据模型辅助 | 模型辅助函数 |

### 2.3 go-boot 核心包参考

go-boot-gorm 依赖 go-boot 的以下核心包：
...
go-boot-gorm 遵循 go-boot 的接口抽象原则，所有集成层通过核心框架中的接口抽象定义，实现运行时互换：

- `data.Transactor` — 事务执行器（GORM DB 实现此接口）
- `data.Transaction` — 事务接口（GORM Transaction 实现此接口）
